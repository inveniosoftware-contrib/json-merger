...
def test_merge_with_nothing():
    m = SkipListsMerger(1, {"some": "other object"}, NOTHING, DictMergerOps.FALLBACK_KEEP_HEAD)
    m.merge()
    assert m.merged_root == {"some": "other object"}

    m = SkipListsMerger(1, NOTHING, {"some": "other object"}, DictMergerOps.FALLBACK_KEEP_HEAD)
    m.merge()
    assert m.merged_root == {"some": "other object"}

    m = SkipListsMerger(
        NOTHING, {"some": "other object"}, NOTHING, DictMergerOps.FALLBACK_KEEP_HEAD
    )
    m.merge()
    assert m.merged_root == {"some": "other object"}
...
def test_conflict_with_custom_ops_update():
    ...
    h = {"r": {"foo": "bab", "spam": "egg"}, "p": {"foo": "bab", "spam": "egg"}}
    u = {"r": {"foo": "bac", "spam": "eggs"}, "p": {"foo": "bac", "spam": "eggs"}}

    # set different strategies compare to the default one
    custom_ops = {"r": DictMergerOps.FALLBACK_KEEP_UPDATE, "p": DictMergerOps.FALLBACK_KEEP_UPDATE}
    expected = {"r": {"foo": "bac", "spam": "eggs"}, "p": {"foo": "bac", "spam": "eggs"}}

    m = SkipListsMerger(r, h, u, DictMergerOps.FALLBACK_KEEP_HEAD, custom_ops=custom_ops)
...
def test_conflict_with_custom_ops_update_and_head():
    ...
    expected = {"r": {"foo": "bac", "spam": "eggs"}, "p": {"foo": "bab", "spam": "egg"}}
    ...
    assert m.conflicts == excinfo.value.content
    assert len(m.conflicts) == 4
    assert Conflict(ConflictType.SET_FIELD, ("p", "spam"), "eggs") in m.conflicts
    assert Conflict(ConflictType.SET_FIELD, ("r", "spam"), "egg") in m.conflicts
    assert Conflict(ConflictType.SET_FIELD, ("r", "foo"), "bab") in m.conflicts
...
def test_conflict_with_custom_ops_update_and_head_mixed():
    ...
    expected = {"r": {"foo": "bac", "spam": "eggs"}, "p": {"foo": "bab", "spam": "eggs"}}
    ...
    assert len(m.conflicts) == 4
    assert Conflict(ConflictType.SET_FIELD, ("r", "spam"), "egg") in m.conflicts
    assert Conflict(ConflictType.SET_FIELD, ("p", "spam"), "egg") in m.conflicts
    assert Conflict(ConflictType.SET_FIELD, ("r", "foo"), "bab") in m.conflicts
    assert Conflict(ConflictType.SET_FIELD, ("p", "foo"), "bac") in m.conflicts
...
def test_conflict_with_custom_ops_update_and_head_with_nested_rules():
    ...
    expected = {"r": {"foo": "bab", "spam": "eggs"}, "p": {"foo": "bab", "spam": "eggs"}}
...
def test_data_lists_nested():
    ...
    m = SkipListsMerger(r, h, u, DictMergerOps.FALLBACK_KEEP_HEAD, data_lists=["r"])
    m.merge()
...
def test_get_custom_strategy():
    patches = [
        Dictdiffer_Conflict(
            ("change", "p.foo", ("baa", "bab")), ("change", "p.foo", ("baa", "bac"))
        ),
        Dictdiffer_Conflict(
            ("change", "p.spam", ("eg", "egg")), ("change", "p.spam", ("eg", "eggs"))
        ),
        Dictdiffer_Conflict(
            ("change", "r.foo", ("baa", "bab")), ("change", "r.foo", ("baa", "bac"))
        ),
        Dictdiffer_Conflict(
            ("change", "r.spam", ("eg", "egg")), ("change", "r.spam", ("eg", "eggs"))
        ),
    ]
    ...
    m = SkipListsMerger({}, {}, {}, DictMergerOps.FALLBACK_KEEP_HEAD, custom_ops=custom_ops)
...
def test_get_custom_strategy_mixed_patches():
    patches = [
        Dictdiffer_Conflict(("change", "b", ("1", "2")), ("remove", "", [("b", "1")])),
        Dictdiffer_Conflict(("add", "", [("a", "1")]), ("add", "", [("a", "d")])),
    ]

    custom_ops = {"b": DictMergerOps.FALLBACK_KEEP_UPDATE, "a": DictMergerOps.FALLBACK_KEEP_UPDATE}
...
def test_get_custom_strategy_mixed_patches_with_nested_changes():
    patch = Dictdiffer_Conflict(("change", "a.b", ("1", "2")), ("change", "a.b", ("1", "3")))
...
def test_get_custom_strategy_change_remove_patch():
    patch = Dictdiffer_Conflict(("change", "a.b", ("1", "2")), ("remove", "", [("a", {"b": "1"})]))
...
def test_get_custom_strategy_change_remove_add_patches_multiple_changes():
    patches = [
        Dictdiffer_Conflict(
            ("change", "a.c", ("2", "4")), ("remove", "", [("a", {"c": "2", "b": "1"})])
        ),
        Dictdiffer_Conflict(
            ("change", "a.b", ("1", "3")), ("remove", "", [("a", {"c": "2", "b": "1"})])
        ),
        Dictdiffer_Conflict(
            ("add", "a", [("d", "5")]), ("remove", "", [("a", {"c": "2", "b": "1"})])
        ),
        Dictdiffer_Conflict(
            ("add", "a", [("z", "7")]), ("remove", "", [("a", {"c": "2", "b": "1"})])
        ),
    ]
...
def test_get_custom_strategy_add_patch_very_nested():
    patch = Dictdiffer_Conflict(("add", "a.b.c", [("e", "2")]), ("add", "a.b.c", [("e", "3")]))
...
def test_get_custom_strategy_add_change_patches_very_nested():
    patches = [
        Dictdiffer_Conflict(("change", "a.b.c.d", ("1", "4")), ("change", "a.b.c.d", ("1", "5"))),
        Dictdiffer_Conflict(("add", "a.b.c", [("e", "2")]), ("add", "a.b.c", [("e", "3")])),
    ]
...
def test_get_custom_strategy_add_patch_in_the_root():
    patch = Dictdiffer_Conflict(("add", "", [("a", "1")]), ("add", "", [("a", "2")]))
...
def test_get_custom_strategy_mixed_patch_9():
    patch = Dictdiffer_Conflict(("add", "a", [("b", "1")]), ("add", "a", [("b", "2")]))

    custom_ops = {"a": DictMergerOps.FALLBACK_KEEP_UPDATE, "a.b": DictMergerOps.FALLBACK_KEEP_HEAD}
...
def test_get_custom_strategy_mixed_patches_with_nesting_after_list():
    patches = [
        ...
        Dictdiffer_Conflict(
            ("add", ["a", "c", 0, "b"], [("z", "5")]), ("add", ["a", "c", 0, "b"], [("z", "6")])
        ),
    ]
...
def test_get_rule_for_field_perfect_match():
    custom_ops = {"a.b.c.d": DictMergerOps.FALLBACK_KEEP_UPDATE}
...
def test_get_rule_for_field_uses_key_path():
    ...
    m = SkipListsMerger(
        {}, {}, {}, DictMergerOps.FALLBACK_KEEP_HEAD, custom_ops=custom_ops, key_path=["a", 0]
    )
...
def test_get_rule_for_field_looks_at_correct_path():
    ...
    m = SkipListsMerger(
        {}, head, update, DictMergerOps.FALLBACK_KEEP_HEAD, custom_ops=custom_ops, data_lists="a.b"
    )
